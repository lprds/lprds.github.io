...
**RLS policy-level test 2026-07-29 — 14/14 PASS, but NOT yet "verified" (Danielle's correction 2026-07-30).** Two throwaway households, attacks run under the `authenticated` role with **SQL-simulated JWT claims** (`set_config('request.jwt.claims', ...)`): kid cannot read another household's rows or a sibling's transactions; cannot self-approve, self-gift, update/delete ledger rows, create tasks, claim as a sibling, or change the auto-approve window; parent is scoped to their own household. Positive paths pass (kid claims own task; parent approval posts payout).

**What that did NOT test — the real consumer path.** Simulated claims exercise the policies but bypass the actual stack: real Supabase Auth sign-in → real signed JWT → Data API (PostgREST) → RLS. Per the standing rule, "verified" = the real consumer's path, never a self-minted auth shortcut. **The RLS gate on real data stays OPEN** until the attack battery is re-run through the Data API with real logged-in sessions.

**Known front-end prerequisites for that real-path test:**
1. **Expose the `pistachio` schema in the project's Data API settings** (PostgREST serves only explicitly exposed schemas; default is `public`/`graphql_public`, so `pistachio` is expected to be unreachable via supabase-js until this is done). Unverified-but-expected; confirm against the live API.
2. Real auth users (waits on the kid-auth decision), signed in through Supabase Auth, driving the same attack battery via PostgREST with the anon key + session tokens.
