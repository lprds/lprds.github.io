...
// The hard-deadline lane is valuable because it is scarce (SPEC-v2 §1).
const MAX_HARD_DEADLINES = 3;
...
async function snapshot(session: Session) {
  ...
  const [household, members, live, recent, kudos, events] = await Promise.all([
    ...
    // declined + renegotiate ride with the live set, unwindowed: the "needs
    // another plan" pile must never quietly lose an unresolved item (§3).
    db.from("tasks").select("*")
      .eq("household_id", session.householdId)
      .in("status", ["requested", "open", "declined", "renegotiate"])
      .order("created_at"),
    db.from("tasks").select("*")
      .eq("household_id", session.householdId)
      .eq("status", "done")
...
const ops: Record<string, (body: Record<string, unknown>, ip: string) => Promise<unknown>> = {
  ...
  "task.save": async (body) => {
    ...
    // v2 fields ride along only when the client sent them — an edit from a
    // client that predates them must never null out what's stored.
    if ("answer_by" in input) row.answer_by = timestampOrNull(input.answer_by);
    if ("deadline_on" in input) row.deadline_on = dateOrNull(input.deadline_on);
    if ("hard_deadline" in input) {
      row.hard_deadline = input.hard_deadline === true;
      if (row.hard_deadline && !(existing?.hard_deadline)) {
        const { count: hardCount } = await db.from("tasks")
          .select("id", { count: "exact", head: true })
          .eq("household_id", session.householdId)
          .eq("hard_deadline", true)
          .in("status", ["requested", "open", "renegotiate"])
          .neq("id", id);
        if ((hardCount ?? 0) >= MAX_HARD_DEADLINES) {
          throw new ApiError("invalid",
            `You've got ${MAX_HARD_DEADLINES} already. Which one stops being a hard deadline?`);
        }
      }
    }
    ...
    if (existing) {
      ...
      let kind = "edited";
      // Reassigning a live task to someone else is an ask, not an assignment. It
      // re-enters the request flow instead of silently landing on their list.
      if ((existing.status === "open" || existing.status === "requested") && ownerId !== existing.owner_id) {
        if (ownerId && ownerId !== session.actorId) {
          patch.status = "requested";
          patch.requested_by = session.actorId;
      ...
      if (error) throw new ApiError("server", error.message, 500);
      await logEvent(session.householdId, id, session.actorId, kind, { title: row.title, owner_id: ownerId });
      if (row.hard_deadline === true && !existing.hard_deadline) {
        await logEvent(session.householdId, id, session.actorId, "deadline_set",
          { deadline_on: row.deadline_on ?? existing.deadline_on });
      }
      return { task: data };
    }
  ...
  // Idempotent, owner only, first write wins. Feeds "Seen 2h ago" on the other
  // board — reassurance, never evidence (§1 rung 1).
  "task.seen": async (body) => {
    const session = await authenticate(body);
    const task = await loadTask(session.householdId, body.id);
    if (task.owner_id !== session.actorId) {
      throw new ApiError("forbidden", "Only the person being asked can mark this seen.", 403);
    }
    if (task.seen_at) return { task };
    const { data, error } = await db.from("tasks")
      .update({ seen_at: new Date().toISOString() })
      .eq("id", task.id).is("seen_at", null).select("*").maybeSingle();
    if (error) throw new ApiError("server", error.message, 500);
    if (data) await logEvent(session.householdId, task.id, session.actorId, "seen", {});
    return { task: data ?? task };
  },

  // The answer between yes and no (§2): "not me — let's find another way".
  // Parks the item in the "needs another plan" pile with a suggested route.
  "task.renegotiate": async (body) => {
    const session = await authenticate(body);
    const task = await loadTask(session.householdId, body.id);
    if (task.status !== "requested") {
      throw new ApiError("invalid", "That item isn't waiting on an answer.");
    }
    if (task.owner_id !== session.actorId) {
      throw new ApiError("forbidden", "Only the person being asked can answer this.", 403);
    }
    const route = oneOf(body.route, ["swap", "pay", "drop", "talk"] as const, "talk");
    const note = str(body.note, "Note", 300, false);
    const { data, error } = await db.from("tasks").update({
      status: "renegotiate",
      renegotiate_route: route,
      renegotiate_note: note || null,
    }).eq("id", task.id).select("*").single();
    if (error) throw new ApiError("server", error.message, 500);
    await logEvent(session.householdId, task.id, session.actorId, "renegotiated", { route, note });
    return { task: data };
  },

  // A visible way out of the pile (§3). Dropping requires two distinct people:
  // the first tap proposes, the second confirms. Nothing vanishes because one
  // person quietly decided it didn't matter.
  "task.resolve": async (body) => {
    const session = await authenticate(body);
    const task = await loadTask(session.householdId, body.id);
    if (task.status !== "declined" && task.status !== "renegotiate") {
      throw new ApiError("invalid", "That item isn't in the pile.");
    }
    const outcome = String(body.outcome ?? "");

    if (outcome === "taken_back") {
      const { data, error } = await db.from("tasks").update({
        status: "open",
        owner_id: session.actorId,
        outcome: "taken_back",
        decline_reason: null,
        renegotiate_route: null,
        renegotiate_note: null,
        drop_proposed_by: null,
        drop_agreed_by: null,
      }).eq("id", task.id).select("*").single();
      if (error) throw new ApiError("server", error.message, 500);
      await logEvent(session.householdId, task.id, session.actorId, "resolved", { outcome });
      return { task: data };
    }

    if (outcome === "outsourced") {
      const { data, error } = await db.from("tasks")
        .update({ status: "archived", outcome: "outsourced" })
        .eq("id", task.id).select("*").single();
      if (error) throw new ApiError("server", error.message, 500);
      await logEvent(session.householdId, task.id, session.actorId, "resolved", { outcome });
      return { task: data };
    ...
    if (outcome === "dropped") {
      if (!task.drop_proposed_by) {
        const { data, error } = await db.from("tasks")
          .update({ drop_proposed_by: session.actorId })
          .eq("id", task.id).select("*").single();
        if (error) throw new ApiError("server", error.message, 500);
        await logEvent(session.householdId, task.id, session.actorId, "drop_proposed", {});
        return { task: data };
      }
      if (task.drop_proposed_by === session.actorId) {
        throw new ApiError("invalid", "You've proposed letting this go. It takes both of you to drop it.");
      }
      const { data, error } = await db.from("tasks").update({
        status: "archived",
        outcome: "dropped",
        drop_agreed_by: session.actorId,
      }).eq("id", task.id).select("*").single();
      if (error) throw new ApiError("server", error.message, 500);
      await logEvent(session.householdId, task.id, session.actorId, "resolved", { outcome });
      return { task: data };
    ...
    throw new ApiError("invalid", "Unknown outcome.");
  },
  ...
  "task.accept": async (body) => {
    ...
    const { data, error } = await db.from("tasks").update(patch)
      .eq("id", task.id).select("*").single();
    if (error) throw new ApiError("server", error.message, 500);
    // "Yes, but not this week" is its own event, so the asking side reads
    // "agreed for later" rather than an ambiguous future date (§2).
    await logEvent(session.householdId, task.id, session.actorId,
      body.later === true ? "accepted_later" : "accepted",
      { due_on: patch.due_on ?? task.due_on });
    return { task: data };
  },
  ...
  "task.reopen": async (body) => {
    ...
    const { data, error } = await db.from("tasks")
      .update({
        status: "open", completed_at: null, completed_by: null, decline_reason: null,
        renegotiate_route: null, renegotiate_note: null,
        drop_proposed_by: null, drop_agreed_by: null, outcome: null,
      })
      .eq("id", task.id).select("*").single();
    if (error) throw new ApiError("server", error.message, 500);
  ...
  "task.defer": async (body) => {
    ...
    switch (String(body.to ?? "tomorrow")) {
      ...
      case "weekend": {
        let d = base;
        do { d = addDays(d, 1); } while (parseDay(d).getUTCDay() !== 6);
        due = d;
        break;
      }
